...
public class Project1 {
	...
	public void startProject1() {
		...
		int intscanner = 0; // 숫자 스캐너
		String sscanner = null; // 문자열 스캐너
		int seatscanner = 0;
		// -----------------------------------------------------------------------
				...
				while (true) {
					...
					if (choice1 == 1) {
						...
						for (ConInfoVO vo : clist) {
							System.out.println(String.format("%-1s\t%-24s\t%-5s\t%-5s\t%-10s\t%-24s\t%-10s",
									vo.getConcert_id(), vo.getTitle(), vo.getGenre(), vo.getRunning_time(),
									vo.getConcert_date().substring(0, 10), vo.getLocation(), vo.getTime()));
						}
						System.out.println("---------------------------------------------------------------");
					} else if (choice1 == 0) {
						// 이전 메뉴로 돌아가기
...
//					                System.out.println("현재 회원 정보:");
										System.out.println(String.format("%-8s\t%-18s\t%-15s\t%-10s\t%-2s",
												vo.getUser_id(), vo.getEmail(), vo.getPhonenumber(), vo.getBirthday(),
												vo.getGender()));
									}
							...
							if (select == 2) {
								...
								for (Pay_VO vo : paymentList) {
									System.out.println(String.format("%-1s\t%-12s\t%-10s\t%-10s\t%-12s\t%-10s",
											vo.getPayment_id(), vo.getUser_id(), vo.getPayment_method(),
											vo.getTotal_price(), vo.getPayment_date(), vo.getBook_id()));
								}
							...
							if (select == 3) {
								...
								for (Seat_infoVO vo1 : slist) {
									for (ReservationVO vo : rlist) {
										if (vo.getHall_id() == vo1.getHall_id()
												&& vo.getSeat().equalsIgnoreCase(vo1.getSeat_no())) {
								...
								System.out
										.println("예약ID\t사용자ID\t콘서트ID\t홀ID\t연락처\t\t예매인원\t좌석번호\t좌석가격\t수령방법\t예약날짜\t\t상태");
								for (ReservationVO vo : rlist) {
									System.out.println(String.format(
											"%-1s\t%-7s\t%-1s\t%-1s\t%-7s\t%-1s\t%-5s\t%-1s\t%-1s" + "\t%-1s\t%-51s",
											vo.getBook_id(), vo.getUser_id(), vo.getConcert_id(), vo.getHall_id(),
											vo.getPhonenumber(), vo.getCount(), vo.getSeat(), vo.getPrice(),
											vo.getHowtoget(), vo.getCreateDate(), vo.getStatus()));
									System.out.println(
											"-------------------------------------------------------------------------");
								}
										...
										while (true) {
											...
											if (selectedbook != null) {
												...
												case 2:
													...
													ConInfoVO id = conInfoDAO
															.selectOne(String.valueOf(selectedbook.getConcert_id()));

													for (Seat_infoVO vo : slist) {
														if (vo.getHall_id() == id.getHall_id()) {
															System.out.println(
																	vo.getSeat_id() + "." + vo.getSeat_no() + " ");
													...
													while (true) {
														...
														Seat_infoVO selectseat = seatInfoDAO
																.selectOne(String.valueOf(seatscanner));
														selectedbook.setSeat(selectseat.getSeat_no());

														if (seatscanner < seatidabouthall.get(0)
																|| seatscanner > seatidabouthall
																		.get(seatidabouthall.size() - 1)) {
									...
									if (intscanner == 2) {
										...
										while (true) {
											...
											if (selectedbook != null) {
												...
												System.out.println(
														"예약번호 " + selectedbook.getBook_id() + "번의 예매정보가 삭제되었습니다.");
												break;
											} else {
